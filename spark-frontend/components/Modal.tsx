        ...
        if (modalContent.operation === "create") {
          ...
          const downloadURL =
            eventToDisplay && !photoURL
              ? eventToDisplay.photoURL
              : await uploadImageAsync(photoURL, "events");
          eventData.imageURL = downloadURL; // append to event data before sending
          ...
          >
            {currentUser && currentUser.email !== adminEmail ? (
              ...
              eventProviders.map((eventProvider) => {
                return (
                  <option key={eventProvider.id} value={eventProvider.name}>
                    {eventProvider.name}
                  </option>
                );
              })
          ...
          >
            {eventCategories.length > 0
              ? eventCategories.map((eventCategory) => {
                  return (
                    <option key={eventCategory.id} value={eventCategory.name}>
                      {eventCategory.name}
                    </option>
                  );
                })
    ...
    const PrefectureInput = () => {
      return (
        <>
          ...
          <select name="prefecture" id="prefecture-input" ref={prefectureRef}>
    ...
    const PhotoPreview = () => {
      return eventToDisplay && modalContent.operation === "edit" ? (
        !photoURL && eventToDisplay.imageURL !== null ? (
          ...
          <Image src={photoURL} alt="" width={400} height={400} />
        ) : (
